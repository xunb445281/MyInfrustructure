...
namespace Standard.Infrastructure.Helpers
{
    ...
    public static class Enum
    {
        ...
        /// </summary>
        /// <typeparam name="TEnum">枚举类型</typeparam>
        public static List<ListItem> GetItems<TEnum>()
        {
        ...
        /// 添加描述项
        /// </summary>
        private static void AddItem<TEnum>(ICollection<ListItem> result, FieldInfo field)
        {
            if (!field.FieldType.GetTypeInfo().IsEnum)
                return;
            var value = GetValue<TEnum>(field.Name);
            var description = Reflection.GetDescription(field);
            result.Add(new ListItem(description, value.ToString(), value));
        }
    }
}
